...
Raw texture submissions are rate-limited and use three preconfigured overlay handles as a true ring. At most the newest two opaque generations remain visible with explicit sort orders. New pixels are uploaded only to the fully hidden third handle; after a compositor grace frame it is shown above the current top layer, then another synchronized frame passes before the oldest layer is hidden and recycled. No visible layer is rewritten during an ordinary update. A transient `RequestFailed` response preserves the current top layer and retries at a bounded rate. If all three handles remain stuck, a fresh pool is created in the same OpenVR session and the old pool is retired only after the replacement top layer is visible. Only repeated non-transient failures rebuild the complete OpenVR session.
